...
export default function CarCard({ car }) {
  const { t, lang, user, toggleFavorite, isFavorite, toggleCompare, isInCompare } = useApp();
  ...
  return (
    <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 16, overflow: 'hidden', transition: 'transform .2s', cursor: 'pointer' }}
      ...
      <div style={{ padding: 18 }}>
        ...
        <div style={{ fontSize: 13, color: 'var(--text3)', marginBottom: 12 }}>{localizeSalonName(car.dealership_name, lang)} · {car.year}</div>
